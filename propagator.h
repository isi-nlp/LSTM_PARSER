...
namespace nplm
{
	...
	public:
		...
		Matrix<precision_type, Dynamic, Dynamic> dummy_zeros, dummy_ones;
	...
	public:
	    propagator() : minibatch_size(0), 
					...
					nce_loss(),
					dummy_zeros(Matrix<precision_type, Dynamic, Dynamic>()),
					dummy_ones(Matrix<precision_type, Dynamic, Dynamic>()){ }
	    ...
	    propagator (model &encoder_lstm, 
	      ...
	      : encoder_plstm(&encoder_lstm),
			...
			output_layer_node(&encoder_lstm.output_layer,minibatch_size),
			...
			combiner(&combiner),
			dummy_zeros(Matrix<precision_type, Dynamic, Dynamic>::Zero(encoder_lstm.output_layer.n_inputs(), minibatch_size)),
			dummy_ones(Matrix<precision_type, Dynamic, Dynamic>::Ones(encoder_lstm.output_layer.n_inputs(), minibatch_size))
			{
		...
		void resizeEncoderNodesDropout(int minibatch_size, precision_type dropout_probability){
  		  for (int i=0; i<encoder_lstm_nodes.size(); i++){
			  encoder_lstm_nodes[i].resizeDropout(minibatch_size, dropout_probability);
  		  }			
		}	
			
		void resizeDecoderInputsDropout(int minibatch_size, precision_type dropout_probability){
  		  for (int i=0; i<decoder_lstm_nodes.size(); i++){
  			  decoder_input_nodes[i].resizeDropout(minibatch_size, dropout_probability);
  			  decoder_lstm_nodes[i].set_input_node(decoder_input_nodes[i]);
  		  }			
		}
		
		void resizeDecoderNodesDropout(int minibatch_size, precision_type dropout_probability){
  		  for (int i=0; i<decoder_lstm_nodes.size(); i++){
			  decoder_lstm_nodes[i].resizeDropout(minibatch_size, dropout_probability);
  		  }			
		}				
	    ...
	    void resizeDropout(int minibatch_size, 
				precision_type p_drop_input,
				precision_type p_drop_hidden,
				precision_type p_drop_output) {
			this->minibatch_size = minibatch_size;
			resizeOutput(minibatch_size);
			//resizeEncoderNodes(minibatch_size);
			//resizeDecoderNodes(minibatch_size);
			resizeEncoderNodesDropout(minibatch_size, p_drop_hidden);
			resizeDecoderNodesDropout(minibatch_size, p_drop_hidden);
			resizeEncoderInputsDropout(minibatch_size, p_drop_input);
			resizeDecoderInputsDropout(minibatch_size, p_drop_input);
			resizeLosses(minibatch_size);
			resizeRest(minibatch_size);
			resizeOutputDropoutLayers(minibatch_size, p_drop_output);
		}		
				...
				const Eigen::ArrayBase<DerivedS> &sequence_cont_indices)
	    {
			...
			for (int i=output_sent_len-1; i>=0; i--){
				...
				//Zeroing out hidden states based on the sentence continuation vectors.
				zeroOutState(encoder_lstm_nodes[i].h_t,
							sequence_cont_indices.row(i));
				zeroOutState(encoder_lstm_nodes[i].c_t,
							sequence_cont_indices.row(i));										
			}			
				...
				Engine &eng)
	    {
			...
			//Going over the output sentence to generate the hidden states
			for (int i=output_sent_len-1; i>=0; i--){
				//cerr<<" encoder sequence_cont_indices.row(i)) "<<sequence_cont_indices.row(i)<<endl;
				//cerr<<"i is"<<i<<endl;
				if (i==output_sent_len) {
					//NEED TO CHECK THIS!! YOU SHOULD JUST TAKE THE HIDDEN STATE FROM THE LAST POSITION
					//decoder_lstm_nodes[i].copyToHiddenStates(const_current_h,const_current_c);//,sequence_cont_indices.row(i));
					//cerr<<"const_current_c"<<endl<<const_current_c<<endl;
					//cerr<<"const_current_h"<<endl<<const_current_h<<endl;
					/*
					encoder_lstm_nodes[i].filterStatesAndErrors(const_current_h,
																const_current_c,
																encoder_lstm_nodes[i].h_t_minus_one,
					...
					*/											
					...
					encoder_lstm_nodes[i].filterStatesAndErrors(encoder_lstm_nodes[i+1].h_t,
																...
																sequence_cont_indices.row(i));					
					*/							
					//cerr<<"encoder_lstm_nodes[i+1].h_t"<<endl<<encoder_lstm_nodes[i+1].h_t<<endl;
					//cerr<<"encoder_lstm_nodes[i+1].c_t"<<endl<<encoder_lstm_nodes[i+1].c_t<<endl;									
					encoder_lstm_nodes[i].fPropDropout(output_data.row(i), eng,
													   encoder_lstm_nodes[i+1].h_t,
													   encoder_lstm_nodes[i+1].c_t);//,

				}
				//cerr<<"encoder_lstm_nodes[i].h_t"<<endl<<encoder_lstm_nodes[i].h_t<<endl;
				//cerr<<"encoder_lstm_nodes[i].c_t"<<endl<<encoder_lstm_nodes[i].c_t<<endl;					
				zeroOutState(encoder_lstm_nodes[i].h_t,
							sequence_cont_indices.row(i));
				zeroOutState(encoder_lstm_nodes[i].c_t,
							sequence_cont_indices.row(i));						
		...
		/*
		<template typename node_type, typename DerivedH, typename DerivedC, typename DerivedS >
		void copyAndFPropBlock(node_type &node, 
								)
		*/
				...
				const Eigen::ArrayBase<DerivedS> &sequence_cont_indices)
	    {
			...
			for (int i=0; i<output_sent_len; i++){
				...
				if (i==0) {
					...
					*/											
					decoder_lstm_nodes[i].fProp(output_data.row(i),
											decoder_lstm_nodes[i-1].h_t,
											decoder_lstm_nodes[i-1].c_t);//,
					
										//(encoder_lstm_nodes[i-1].c_t.array().rowwise()*sequence_cont_indices.row(i-1)).matrix(),
										//	(encoder_lstm_nodes[i-1].h_t.array().rowwise()*sequence_cont_indices.row(i-1)).matrix());
				}
				zeroOutState(decoder_lstm_nodes[i].h_t,
							sequence_cont_indices.row(i));
				zeroOutState(decoder_lstm_nodes[i].c_t,
							sequence_cont_indices.row(i));						
		...
		void computeLosses(const MatrixBase<DerivedOut> &output,
			 ...
			 //SoftmaxNCELoss<multinomial<data_type> > &softmax_nce_loss){
	 			...
	 			//dummy_zeros.setZero(output_layer_node.param->n_inputs(),minibatch_size);
				...
				//cerr<<"Sent len is "<<sent_len<<endl;
	 			for (int i=sent_len-1; i>=0; i--) {
					...
					bidirectional_combiner_nodes[i].bProp(
						losses[i].d_Err_t_d_h_t.leftCols(current_minibatch_size),
						decoder_lstm_nodes[i].h_t.leftCols(current_minibatch_size),
						encoder_lstm_nodes[i].h_t.leftCols(current_minibatch_size));
	 			}
	 			...
	 			for (int i=sent_len-1; i>=0; i--) {
					...
					precision_type minibatch_log_likelihood;
	 				if (loss_function == LogLoss) {
						...
						//output_dropout_layers[i].fProp(encoder_lstm_nodes[i].h_t,rng);
						output_dropout_layers[i].fProp(
							bidirectional_combiner_nodes[i].combination_layer_node.fProp_matrix,
							rng);	
	 					//First doing fProp for the output layer
	 					//The number of columns in scores will be the current minibatch size
	    ...
	    {	
			...
			for (int i=input_sent_len-1; i>=0; i--) {
				//We need to zero out the backproped derivatives from the future if the sentence continuation vector says
				if (i==0 && input_sent_len-1 > 0) {
				    decoder_lstm_nodes[i].bProp(input_data.row(i),
							   //init_h,
				   			   //init_c,
								bidirectional_combiner_nodes[i].forward_layer_transformation_node.bProp_matrix,
							   //input_layer_node.bProp_matrix,
				   			   decoder_lstm_nodes[i+1].d_Err_t_to_n_d_c_tMinusOne,
							   decoder_lstm_nodes[i+1].d_Err_t_to_n_d_h_tMinusOne,
							   //ASSUMING THAT THE START VECTORS ARE ALL 0. THIS MIGHT NOT BE THE CASE
							   dummy_zeros,
							   dummy_zeros,
							   gradient_check,
							   norm_clipping);	
				    ...
				    decoder_lstm_nodes[i].bProp(input_data.row(i),
							   bidirectional_combiner_nodes[i].forward_layer_transformation_node.bProp_matrix,
							   //input_layer_node.bProp_matrix,
				   			   dummy_zeros, //for the last lstm node, I just need to supply a bunch of zeros as the gradient of the future
				   			   dummy_zeros,
				   			   decoder_lstm_nodes[i-1].h_t,
							   decoder_lstm_nodes[i-1].c_t,									   
							   gradient_check,
	    ...
	    {			
			...
			for (int i=input_sent_len-1; i>=0; i--) {
				/*
				if (i <= input_sent_len -2){
				   cerr<<" i is "<<i<<endl;
	   			   cerr<<" decoder_lstm_nodes[i+1].d_Err_t_to_n_d_c_tMinusOne "<<endl<<decoder_lstm_nodes[i+1].d_Err_t_to_n_d_c_tMinusOne<<endl;
				   cerr<<" decoder_lstm_nodes[i+1].d_Err_t_to_n_d_h_tMinusOne "<<endl<<decoder_lstm_nodes[i+1].d_Err_t_to_n_d_h_tMinusOne<<endl;
				}
				*/
				//cerr<<"i in backprop decoder dropout is "<<i<<endl;
				if (i==0 && input_sent_len-1 > 0) {
					
				    decoder_lstm_nodes[i].bPropDropout(input_data.row(i),
								bidirectional_combiner_nodes[i].forward_layer_transformation_node.bProp_matrix,
							   //input_layer_node.bProp_matrix,
				   			   decoder_lstm_nodes[i+1].d_Err_t_to_n_d_c_tMinusOne,
							   decoder_lstm_nodes[i+1].d_Err_t_to_n_d_h_tMinusOne,
					...
					//cerr<<"decoder_lstm_nodes[i-1].c_t.rows() "<<decoder_lstm_nodes[i-1].c_t.rows()<<
					//	" decoder_lstm_nodes[i-1].c_t.cols() "<<decoder_lstm_nodes[i-1].c_t.cols()<<endl;
				    decoder_lstm_nodes[i].bPropDropout(input_data.row(i),
							   bidirectional_combiner_nodes[i].forward_layer_transformation_node.bProp_matrix,
							   //input_layer_node.bProp_matrix,
				   			   dummy_zeros, //for the last lstm node, I just need to supply a bunch of zeros as the gradient of the future
    ...
    // Dense version (for standard log-likelihood)
    template <typename DerivedOut, typename DerivedS> //, typename DerivedC, typename DerivedH, typename DerivedS>
    void bPropEncoder(const MatrixBase<DerivedOut> &input_data,
		 bool gradient_check,
		 bool norm_clipping,
		 const Eigen::ArrayBase<DerivedS> &sequence_cont_indices)
    {	
		...
		//first getting encoder loss
		for (int i=0; i<input_sent_len; i++) {
			//We need to zero out the backpropagated derivatives from the future if the sequence_cont_indices says so.
			if (i >= 1) { 
				/*
				cerr<<"encoder_lstm_nodes[i-1].d_Err_t_to_n_d_c_tMinusOne "<<endl<<
					encoder_lstm_nodes[i-1].d_Err_t_to_n_d_c_tMinusOne<<endl;
				cerr<<"encoder_lstm_nodes[i-1].d_Err_t_to_n_d_h_tMinusOne "<<endl<<
					encoder_lstm_nodes[i-1].d_Err_t_to_n_d_h_tMinusOne<<endl;	
				*/					
				zeroOutState(encoder_lstm_nodes[i-1].d_Err_t_to_n_d_c_tMinusOne,
							sequence_cont_indices.row(i));
				zeroOutState(encoder_lstm_nodes[i-1].d_Err_t_to_n_d_h_tMinusOne,
							sequence_cont_indices.row(i));	
				/*
				cerr<<"encoder_lstm_nodes[i-1].d_Err_t_to_n_d_c_tMinusOne "<<endl<<
					encoder_lstm_nodes[i-1].d_Err_t_to_n_d_c_tMinusOne<<endl;
  ...
  // Dense version (for standard log-likelihood)
  template <typename DerivedOut, typename DerivedS> //, typename DerivedC, typename DerivedH, typename DerivedS>
  void bPropEncoderDropout(const MatrixBase<DerivedOut> &input_data,
	 bool gradient_check,
	 bool norm_clipping,
	 const Eigen::ArrayBase<DerivedS> &sequence_cont_indices)
	  {			
		...
		for (int i=0; i<input_sent_len; i++) {
			...
			//cerr<<"i in backprop encoder dropout is "<<i<<endl;
			if (i==input_sent_len-1 && input_sent_len-1 > 0) {
	 ...
	 precision_type getGradSqdNorm(){
		...
		precision_type grad_norm = 0;
 		...
 		grad_norm += encoder_plstm->output_layer.getGradSqdNorm();
		...
		grad_norm += combiner->getGradSqdNorm();
		return(grad_norm);
	 ...
	 void updateParams(precision_type learning_rate,
		...
		decoder_plstm->updateParams(learning_rate,
										...
										norm_clipping,
										norm_threshold);	
		combiner->updateParams(learning_rate,
										current_minibatch_size,
										momentum,
										L2_reg,
										norm_clipping,
	  ...
	  {	
			...
			//Matrix<precision_type,Dynamic,Dynamic> dummy_zeros;
			//Right now, I'm setting the dimension of dummy zero to the output embedding dimension becase everything has the 
			//same dimension in and LSTM. this might not be a good idea
			//dummy_zeros.setZero(output_layer_node.param->n_inputs(),current_minibatch_size);
	  ...
	  void computeProbsLog(const MatrixBase<DerivedOut> &output,
						...
						vector<vector<int> > & predicted_labels) 
	  {	
		  	//cerr<<"output is "<<output<<endl;
			//cerr<<"In computeProbs..."<<endl;
			int current_minibatch_size = output.cols();
			...
			for (int i=sent_len-1; i>=0; i--) {
	            ...
	            for (int minibatch_instance=0; minibatch_instance < current_minibatch_size; minibatch_instance++){
					...
					//
					if (output(i,minibatch_instance) != -1){
						predicted_labels[minibatch_instance].push_back(max_index);
					}
	            }				
			}
				 ...
				 
    {
		...
		*/
		...
		//Doing gradient check for the input nodes
 		...
 		paramGradientCheck(input,decoder_input, 
							 decoder_output,
							 (dynamic_cast<input_model_type*>(encoder_plstm->input))->W_x_to_i,
		...
		paramGradientCheck(input,
					 decoder_input, 
					 decoder_output,
					 combiner->forward_layer_transformation_layer,
					 "Combiner: forward_layer_transformation_layer", 
						 init_c,
						 init_h,
						 unigram,
 		...
 		paramGradientCheck(input,
 					 decoder_input, 
 					 decoder_output,
 					 combiner->backward_layer_transformation_layer,
 					 "Combiner: backward_layer_transformation_layer", 
 						 init_c,
 						 init_h,
 						 unigram,
    ...
    void getFiniteDiff(const MatrixBase<DerivedIn> &input,
			 ...
			 bool arg_run_lm) {
				...
				if (dropout_probability > 0) {
					fPropEncoderDropout(decoder_input,
									init_c,
									init_h,
									output_sequence_cont_indices,
									init_rng);	
	...
	void zeroOutState(const MatrixBase<Derived> &const_state,
							const Eigen::ArrayBase<DerivedS> &sequence_cont_indices){
